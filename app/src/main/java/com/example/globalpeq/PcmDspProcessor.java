...
final class PcmDspProcessor {
    ...
    private static final class LegacyPsychoacousticBassProcessor {
        ...
        private float[] leftChannelBuffer = new float[0];
        private float[] rightChannelBuffer = new float[0];

        // ---------- 控制参数 ----------
        private float harmonicMix;      // 谐波混合量 [0..1]
        private float inputDrive;       // 输入激励 [0..5.5]
        private float originalLowMix;   // 原始低频保留量 [0..1]

        // ---------- 内部状态，避免反复创建对象 ----------
        private float[][] tempMonoLow = new float[2][];   // 用于临时存储每个滤波器的输出

        // 动态包络与暴走参数控制状态（新增私有状态，不破坏对外接口）
        private float envSlow = 0.0f;
        private float beastAmount = 0.0f;
        private int activeCutoffHz = 60;

        LegacyPsychoacousticBassProcessor(int sampleRate, int channelCount) {
        ...
        /**
         * 配置虚拟低音参数。<br>
         * <b>注意：</b>本实现忽略 lowCpuMode（始终使用标准模式），
         * 但保留该参数以保证接口兼容。
         */
        void configure(int virtualCutoffHz,
            ...
            // ---------- 频率选择 ----------
            int cutoffHz = (dspAmount > 0.5f) ? dspCutoffHz : virtualCutoffHz;
            if (cutoffHz <= 0) cutoffHz = 60;   // 默认60Hz
            cutoffHz = Math.max(40, Math.min(220, cutoffHz)); // 限制范围
            this.activeCutoffHz = cutoffHz;

            // ---------- 【重构：双段暴走野兽映射】 ----------
            // 保证在 60% 时效果已经极强（beastAmount 达到 1.75），拉到 100% 狂飙至 5.0，直接顶飞！
            this.beastAmount = (float)(Math.pow(virtualAmount, 0.5) * 1.20f + Math.pow(virtualAmount, 3.0) * 3.80f);

            // 谐波混合比例与驱动系数
            this.harmonicMix = beastAmount * 1.2f;
            this.inputDrive = 1.0f + beastAmount * 4.0f; // 拉满时前级驱动高达 21 倍，压榨极致声压

            // ---------- 【重构：不要切死 cutoff 以下的原声】 ----------
            // 原始低音 100% 完整保留（如果开启物理 crossover 分频 dspAmount > 0，才适度增大低音比重）
            this.originalLowMix = (dspAmount > 0f) ? (1.0f + dspAmount * 0.5f) : 1.0f;

            // 重置内部包络，防止切歌爆音
            this.envSlow = 0.0f;

            // 根据采样率与截止频率重新初始化所有双二阶滤波器
            float Q = 0.707f;  // 巴特沃斯

            // ---------- 【重构：子八度频带精确对齐】 ----------
            // 种子低通：切在 cutoffHz 处，完美捕获子八度及以下的全部火种能量，彻底解决“完全没低频”的Bug！
            float lpCut = cutoffHz;
            sourceLowPass1 = new MonoBiquad(BiquadType.LOWPASS, lpCut, Q, sampleRate);
            sourceLowPass2 = new MonoBiquad(BiquadType.LOWPASS, lpCut, Q, sampleRate);

            // 主通道高通：
            // 纯虚拟低音模式下，主通道高通固定设在 20Hz（仅滤除直流），100% 原汁原味旁路原声低音，耳机听感极度丰满！
            // 只有开启物理分频时，才温和切在分频点
            float mainHpCut = (dspAmount > 0f) ? (cutoffHz * 0.8f) : 20.0f;
            mainHpCut = Math.max(20.0f, mainHpCut);

            int filterLength = Math.max(1, channelCount);
            if (mainHighPassL1 == null || mainHighPassL1.length < filterLength) {
                mainHighPassL1 = new MonoBiquad[filterLength];
                mainHighPassL2 = new MonoBiquad[filterLength];
                mainHighPassR1 = new MonoBiquad[filterLength];
                mainHighPassR2 = new MonoBiquad[filterLength];
            }
            for (int ch = 0; ch < filterLength; ch++) {
                mainHighPassL1[ch] = new MonoBiquad(BiquadType.HIGHPASS, mainHpCut, Q, sampleRate);
                mainHighPassL2[ch] = new MonoBiquad(BiquadType.HIGHPASS, mainHpCut, Q, sampleRate);
                mainHighPassR1[ch] = new MonoBiquad(BiquadType.HIGHPASS, mainHpCut, Q, sampleRate);
                mainHighPassR2[ch] = new MonoBiquad(BiquadType.HIGHPASS, mainHpCut, Q, sampleRate);
            }

            // 谐波带通：
            // 下限设在 cutoffHz * 0.9f（例如 120Hz 时为 108Hz），保证产生的 120Hz 二倍频黄金谐波能 100% 饱满溢出！
            // 上限放宽到 cutoffHz * 8f（最大 10kHz ），释放松香味十足的高阶泛音！
            float harmonicLowCut = Math.max(30f, cutoffHz * 0.9f);
            float harmonicHighCut = Math.min(10000f, cutoffHz * 8f);

            harmonicHighPass1 = new MonoBiquad(BiquadType.HIGHPASS, harmonicLowCut, Q, sampleRate);
            harmonicHighPass2 = new MonoBiquad(BiquadType.HIGHPASS, harmonicLowCut, Q, sampleRate);
            harmonicLowPass = new MonoBiquad(BiquadType.LOWPASS, harmonicHighCut, Q, sampleRate);
        }

        /**
         * 处理一帧音频数据（立体声 interleaved 或单声道）。<br>
         * 处理后的数据直接写回 input 缓冲区。
         */
        void process(float[] input, int offset, int count) {
            final int frames = count / channelCount;   // 帧数
            if (frames <= 0) return;
            ...
            // ----- 1. 降混为单声道 -----
            if (channelCount == 1) {
                System.arraycopy(input, offset, monoSource, 0, frames);
            } else {
                for (int i = 0; i < frames; i++) {
                    int idx = offset + i * 2;
                    monoSource[i] = (input[idx] + input[idx + 1]) * 0.5f;
                }
            }

            // ----- 2. 双二阶滤波：提取低频基音种子 -----
            for (int i = 0; i < frames; i++) {
                float x = monoSource[i];
                x = sourceLowPass1.process(x);
                x = sourceLowPass2.process(x);
                monoLow[i] = x;
            }

            // 频带等响度电平自适应补偿
            float uScale = this.activeCutoffHz / 60.0f;
            float freqComp = (float)Math.pow(uScale, 1.8); // 1.8次方狂暴电平补偿，解决调高频率后谐波量不够的顽疾

            // ----- 3. 核心：三极电子管非对称谐波发生器 -----
            for (int i = 0; i < frames; i++) {
                float lowVal = monoLow[i];
                float absLow = Math.abs(lowVal);

                // 自适应慢速包络追踪
                float attS = 180.0f / sampleRate;
                float relS = 15.0f / sampleRate;
                if (attS > 0.5f) attS = 0.5f;
                if (relS > 0.1f) relS = 0.1f;
                if (absLow > envSlow) {
                    envSlow += attS * (absLow - envSlow);
                } else {
                ...
                float x_norm = lowVal * normScale;
                float drive = inputDrive * (1.0f + beastAmount * 2.5f);
                float x = x_norm * drive;

                // 100% 安全理性软限幅，杜绝 NaN / 溢出
                float x_clip = x / (1.0f + Math.abs(x));

                // 二、三、四阶切比雪夫纯净泛音列计算
                float x2 = x_clip * x_clip;
                float x3 = x2 * x_clip;
                float x4 = x2 * x2;

                float h2 = 2.0f * x2 - 1.0f;            // 二阶：打底基准
                float h3 = 4.0f * x3 - 3.0f * x_clip;   // 三阶：硬朗质感
                float h4 = 8.0f * x4 - 8.0f * x2 + 1.0f; // 四阶：空气共鸣

                // 真空管非对称软饱和器（产生极其温暖的松香味）
                float tube;
                if (x_clip > 0.0f) {
                    tube = x_clip / (1.0f + 0.2f * x_clip);
                } else {
                    tube = x_clip / (1.0f - 0.7f * x_clip);
                }

                // 泛音列权重动态分配：随 activeCutoffHz 增加自动抬升 H3, H4 的权重
                float w2 = 1.0f;
                ...
                // 最终合成动态谐波
                float rawHarmonics = harmonics * compressedEnv * freqComp * gainComp;

                // 谐波专用非线性软钳位，榨干 RMS 响度
                harmonicBand[i] = rawHarmonics / (1.0f + 0.45f * Math.abs(rawHarmonics));
            }

            // ----- 4. 谐波带通滤波（100% 溢出黄金倍频谐波） -----
            for (int i = 0; i < frames; i++) {
                float x = harmonicBand[i];
                x = harmonicHighPass1.process(x);
                x = harmonicHighPass2.process(x);
                x = harmonicLowPass.process(x);
                harmonicBand[i] = x;
            }

            // ----- 5. 主通道处理（高通滤波，20Hz 安全旁路） -----
            if (channelCount == 1) {
                System.arraycopy(input, offset, leftChannelBuffer, 0, frames);
            } else {
                for (int i = 0; i < frames; i++) {
                    int idx = offset + i * 2;
                    leftChannelBuffer[i] = input[idx];
                    rightChannelBuffer[i] = input[idx + 1];
                }
            }

            for (int i = 0; i < frames; i++) {
                float L = leftChannelBuffer[i];
                float R = rightChannelBuffer[i];
                L = mainHighPassL1[0].process(L);
                L = mainHighPassL2[0].process(L);
                R = mainHighPassR1[0].process(R);
                R = mainHighPassR2[0].process(R);
                leftChannelBuffer[i] = L;
                rightChannelBuffer[i] = R;
            }

            // ----- 6. 最终混合（原始低频 + 谐波 + 旁路主信号） -----
            for (int i = 0; i < frames; i++) {
                float low = monoLow[i] * originalLowMix;                // 原始物理低音（100% 无损保留）
                float harm = harmonicBand[i] * harmonicMix;             // 精准生成的黄金心理声学谐波
                float mainL = leftChannelBuffer[i];
                float mainR = (channelCount == 1) ? mainL : rightChannelBuffer[i];

                // 最终混合
                float outL = mainL + harm + low;
                float outR = (channelCount == 1) ? outL : (mainR + harm + low);
                ...
                if (channelCount == 1) {
                    ...
                    input[offset + i * 2] = outL;
                    input[offset + i * 2 + 1] = outR;
                }
            }
        }
        ...
        private void ensureBufferSize(int frames) {
            if (monoSource.length < frames) {
                monoSource = new float[frames];
                monoLow = new float[frames];
                harmonicBand = new float[frames];
                leftChannelBuffer = new float[frames];
                rightChannelBuffer = new float[frames];
                tempMonoLow[0] = new float[frames];
                tempMonoLow[1] = new float[frames];
            }
        }

        /** C1 连续理性快速软饱和（免超越函数优化版，大幅提升性能，防发热） */
        private static float softClip(float x) {
            final float knee = 0.85f;
            float absX = Math.abs(x);
            if (absX <= knee) {
                return x;
            } else {
                float over = absX - knee;
                float doubleOver = over * 2.0f;
                float bend = doubleOver / (1.0f + doubleOver); // 高速理性逼近 tanh 函数
                float sign = Math.signum(x);
                return sign * (knee + (1f - knee) * bend);
            }
        }

        // ---------- 内部双二阶滤波器实现（标准 Direct Form I） ----------
        private static final class MonoBiquad {
            private float a0, a1, a2, b1, b2;
            private float x1, x2, y1, y2;

            MonoBiquad(BiquadType type, float freq, float Q, int sampleRate) {
                ...
                // 归一化
                double invA0 = 1.0 / a0;
                this.a0 = (float) (b0 * invA0);
                this.a1 = (float) (b1 * invA0);
                this.a2 = (float) (b2 * invA0);
                this.b1 = (float) (a1 * invA0);
                this.b2 = (float) (a2 * invA0);
            }
